...
// 但底圖看得見，總比整片空白好。
// 本機開發請用 http://lvh.me:8080/ 而非 localhost —— lvh.me 解析到 127.0.0.1，
// 且已登記在 CARTO 的 referer 允許清單中，這樣本機也不會有浮水印。
const CARTO_KEY_HOSTS = (process.env.VUE_APP_CARTO_KEY_HOSTS || 'wenlab501.github.io,lvh.me')
  .split(',')
  .map((host) => host.trim())
  .filter(Boolean);
